...
export default function HomePage() {
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        ...
        if (heroData) setHeroContent((p) => ({ ...p, ...(heroData as any) }));
        if (aboutData) setAboutContent((p) => ({ ...p, ...(aboutData as any) }));
  ...
  const previewPkgs = packages.length > 0
    ? packages.filter((p) => p.type === "lengkap").slice(0, 3)
    : [];

  // Services data
  const services = [
    { icon: "💄", title: "Rias & Busana", description: "Tata rias pengantin profesional dengan sentuhan anggun, lengkap dengan busana pengantin pilihan terbaik." },
    { icon: "🌸", title: "Dekorasi Pelaminan", description: "Dekorasi megah yang disesuaikan dengan tema pernikahan Anda, dari klasik hingga modern kontemporer." },
    { icon: "📸", title: "Dokumentasi", description: "Abadikan setiap momen berharga dengan foto dan video berkualitas tinggi oleh tim fotografer berpengalaman." },
    { icon: "🎵", title: "Entertainment", description: "MC profesional, sound system berkualitas, dan hiburan musik untuk memeriahkan acara pernikahan Anda." },
  ];
  ...
  return (
    <main>
      ...
      <section className="hero" id="home">
        <div className="hero-bg" style={{ backgroundImage: `url(${heroContent.bg_image_url})` }} />
      ...
      {/* ═══════ PARALLAX QUOTE ═══════ */}
      <section className="parallax-section" style={{ backgroundImage: `url(${heroContent.parallax_image_url})` }}>
      ...
      <section className="section section-alt" id="paket">
        <div className="container">
          ...
          <div className="pricing-preview-grid">
            {(previewPkgs.length > 0 ? previewPkgs : [
              { id: "1", name: "Silver", price: "Rp 15.000.000", featured: false, sections: [{ features: ["Pelaminan 6 Meter", "Rias Pengantin Lengkap", "MC & Sound System", "Dokumentasi Foto"] }] },
              { id: "2", name: "Gold", price: "Rp 25.000.000", featured: true, sections: [{ features: ["Pelaminan 8 Meter", "Rias Pengantin Premium", "MC & Sound System", "Foto & Video", "Photobooth Corner"] }] },
              { id: "3", name: "Platinum", price: "Rp 35.000.000", featured: false, sections: [{ features: ["Pelaminan 10 Meter Megah", "Rias Pengantin Exclusive", "MC & Full Entertainment", "Foto & Video Cinematic", "Full Dekorasi"] }] },
            ] as any[]).map((pkg: any, i: number) => (
          ...
          <div className="contact-grid">
            ...
            <AnimatedSection className="contact-form-wrapper" direction="right">
              <form onSubmit={handleFormSubmit} className="contact-form">
                ...
                <div className="form-group">
                  <input type="text" placeholder="Lokasi Acara" value={formData.location} onChange={(e) => setFormData({ ...formData, location: e.target.value })} />
                </div>
                <div className="form-group">
